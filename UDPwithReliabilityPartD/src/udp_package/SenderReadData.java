...
public class SenderReadData implements Runnable {
	...
	private void sendData(echoData echo)  {
		...
		try {
			...
			if (random < 25) {
				System.out.println("Data is lost when sending");
			} else {
				send.sendData(echo);
			}
		} catch (ClassNotFoundException e) {
	...
	private void readData() throws FileNotFoundException, IOException, ClassNotFoundException {
		try (BufferedReader br = new BufferedReader(new FileReader("/Users/yingjianwu/Documents/Java/LeetcodePractice/linkedList/UDPwithReliabilityPartA/src/do-re-mi.txt"))) {
		    String line;
