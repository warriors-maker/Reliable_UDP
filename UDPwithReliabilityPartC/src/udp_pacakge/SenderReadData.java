...
public class SenderReadData implements Runnable {
	...
	private void sendData(echoData echo)  {
		...
		try {
			...
			if (random < 25) {
				System.out.println("Data is lost when sending, the lost data sequence is" + echo.getSequenceNumber());
				System.out.println("The sentence lost is " + echo.getSentence());
			} else {
				send.sendData(echo);
			}
		} catch (ClassNotFoundException e) {
	...
	private void readData() throws FileNotFoundException, IOException, ClassNotFoundException {
		try (BufferedReader br = new BufferedReader(new FileReader("/Users/yingjianwu/Documents/Java/LeetcodePractice/linkedList/UDPwithReliabilityPartC/src/do-re-mi.txt"))) {
		    String line;
